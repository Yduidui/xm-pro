...
<!--            //scope 是 Vue 单文件组件 (SFC) 中 el-table 组件的插槽定义scope 是一个对象，它包含了与当前行（el-table 的每一行）相关的数据和信息。-->
    ...
    <el-dialog title="员工信息" v-model="data.formVisible" width="500" destroy-on-close>
      <el-form ref="formRef" :rules="data.rules" :model="data.form" label-width="80px" style="padding-right: 30px">
        ...
        <el-form-item label="工号" >
          <el-input v-model="data.form.jobnum" autocomplete="off" placeholder="请输入工号"/>
        </el-form-item>
...
const update = () =>{
  request.put('/employee/update',data.form).then((res) => {    //编辑的对象有id
    if(res.code === '200'){
      data.formVisible = false;
      ElMessage.success('操作成功')
      load()    //新增后加载最新的数据
